...
def image_reconstruction_loss(decoder_l, image_l, ssim=False, normalize=None):
  """
  ...
  if ssim:
    abs_err = torch.abs(decoder_l - image_l)
    loss = 0.85*SSIM(decoder_l, image_l) + 0.15*abs_err
    return loss.mean(dim=(-3, -2, -1))
  else:
    p_norm = 0.5
    err = torch.abs(decoder_l - image_l)**p_norm
    err = err.mean(dim=(-3, -2, -1))

    if normalize is None:
      return err
    elif normalize == "intensity":
      avg_intensity = image_l.mean(dim=(-3, -2, -1), keepdims=True)
      err_mean_predictor = torch.abs(image_l - avg_intensity)**p_norm
      err_mean_predictor = err_mean_predictor.mean(dim=(-3, -2, -1))
      return err / err_mean_predictor
    elif normalize == "color":
      avg_color = image_l.mean(dim=(-2, -1), keepdims=True)
      err_mean_predictor = torch.abs(image_l - avg_color)**p_norm
      err_mean_predictor = err_mean_predictor.mean(dim=(-3, -2, -1))
      return err / err_mean_predictor
    elif normalize == "stdev":
      return err / image_l.std(dim=(-3, -2, -1))
    else:
      raise NotImplementedError()


def save_data(train_loaders, novel_loaders, output_folder, opt, num_train=1000, num_novel=1000, save_images=True):
  ...
  decoder_net.load_state_dict(torch.load(os.path.join(
      opt.load_weights_folder, "decoder_net.pth")), strict=True)
    ...
    for loader_type, loader_list in [("train", train_loaders), ("novel", novel_loaders)]:
      ...
      for loader in loader_list:
        for i, inputs in enumerate(loader):
          ...
          decoded_l = process_batch(feature_net, decoder_net, inputs["color_l/0"], inputs["color_r/0"], opt)
          loss_batch = image_reconstruction_loss(decoded_l, inputs["color_l/{}".format(opt.decoder_loss_scale)], ssim=opt.ssim, normalize=opt.normalize)

          # if opt.stdev_normalization:
            # stdev_batch = torch.sqrt(torch.var(inputs["color_l/{}".format(opt.decoder_loss_scale)], (-3, -2, -1)))
            # loss_adj_batch = loss_batch / stdev_batch
          # else:
            # loss_adj_batch = loss_batch

          # print("Loss:", loss_batch[0])
          # print("Stdev:", stdev_batch[0])
          # print("Adj Loss:", loss_batch[0] / stdev_batch[0])
          assert(loss_batch.shape[0] == len(inputs["color_l/{}".format(opt.decoder_loss_scale)]))

          if save_images and save_ctr < 10:
            viz_decoded = tensor_to_cv_rgb(decoded_l[0].cpu())
            viz_original = tensor_to_cv_rgb(inputs["color_l/{}".format(opt.decoder_loss_scale)][0].cpu())
            ...
            save_ctr += 1
            # cv.imshow("decoded", viz_decoded)
            # cv.imshow("original", viz_original)
            # cv.waitKey(0)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--decoder_loss_scale", type=int, default=1, help="Scale that the decoder was trained to predict images at")
  parser.add_argument("--batch_size", type=int, default=16, help="Batch size for loading images")
  parser.add_argument("--mode", type=str, choices=["save", "histogram", "pr", "pr_multiple"])
  ...
  if opt.decoder_loss_scale != 0:
    scale_list.append(opt.decoder_loss_scale)
    ...
    if opt.environment == "vk_to_sf":
      train_datasets = [
        StereoDataset("/home/milo/datasets/virtual_kitti", "VirtualKitti", "virtual_kitti_clone",
                      368, 960, "train", scales=scale_list, load_disp_left=False, load_disp_right=False)
      ]
      novel_datasets = [
        StereoDataset("/home/milo/datasets/sceneflow_flying_things_3d", "SceneFlowFlying", "sceneflow_flying",
                    384, 960, "train", scales=scale_list, load_disp_left=False, load_disp_right=False)
      ]
