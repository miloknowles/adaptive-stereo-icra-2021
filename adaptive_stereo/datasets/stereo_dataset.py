...
class StereoDataset(Dataset):
    def __init__(
        ...
        splits_path = os.path.abspath(
            os.path.join(os.path.abspath(__file__), "../../splits/")
        )
        self.dataset = dataset_name
        self.lines = read_lines(
            os.path.join(splits_path, split, "{}_lines.txt".format(subsplit))
        )
    ...
    def transform(self, rgb_l, rgb_r, disp_l, disp_r):
        ...
        if self.random_crop:
            i, j, h, w = transforms.RandomCrop.get_params(
                rgb_l, output_size=(self.height, self.width)
            )
        else:
            i, j = (rgb_l.shape[-2] - self.height) // 2, (
                rgb_l.shape[-1] - self.width
            ) // 2
            h, w = self.height, self.width

        assert self.height <= rgb_l.shape[-2] and self.width <= rgb_l.shape[-1]
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        """
        outputs = {}
        rgb_l_path, rgb_r_path, disp_l_path, disp_r_path = [
            os.path.join(self.dataset_path, p) for p in self.lines[index].split(" ")
        ]
        ...
        for s in self.scales:
            if s != 0:
                ...
                outputs["color_l/{}".format(s)] = F.interpolate(
                    rgb_l.unsqueeze(0),
                    size=size_for_scale,
                    mode="bilinear",
                    align_corners=False,
                ).squeeze(0)
                outputs["color_r/{}".format(s)] = F.interpolate(
                    rgb_r.unsqueeze(0),
                    size=size_for_scale,
                    mode="bilinear",
                    align_corners=False,
    ...
    def get_intrinsics_normalized(self):
        ...
        if self.dataset in ["KittiStereo2012", "KittiStereo2015", "KittiRaw"]:
            return torch.Tensor(
                [[0.5885, 0.0, 0.4972], [0.0, 1.9501, 0.4972], [0.0, 0.0, 1.0]]
            )
        elif "SceneFlow" in self.dataset:
            return torch.Tensor(
                [[1.09375, 0.0, 0.5], [0, 1.94444, 0.5], [0.0, 0.0, 1.0]]
            )
